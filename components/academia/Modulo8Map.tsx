...
import { LESSONS8, MODULO8 } from "@/lib/academia/modulo8";
import { readProgress, isModuleComplete, M1_EVENT, type M1Progress } from "@/lib/academia/progress";

// M8 se desbloquea cuando los 2 slugs de M7 están completados.
const M7_SLUGS = ["que-es-un-profeta", "amos-profeta-social"];

// Mapa del Módulo 8 — las 6 lecciones del Mussar / J1 (semanas 35–40).
export default function Modulo8Map() {
  ...
  const m7Complete = prog ? isModuleComplete(M7_SLUGS) : false;

  const completed = prog?.completed ?? [];
  const seen = prog?.seen ?? [];
  const doneCount = m7Complete
    ? LESSONS8.filter((l) => completed.includes(l.slug)).length
    : 0;
  const pct = (doneCount / MODULO8.total) * 100;

  const current = m7Complete
    ? LESSONS8.find((l) => !completed.includes(l.slug))?.slug ?? null
    : null;
  const m8AllDone = m7Complete && LESSONS8.every((l) => completed.includes(l.slug));
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="mt-4 text-center">
          ...
          <p className="mt-2 font-cinzel text-sm uppercase tracking-[0.3em] text-gold/70">
            {MODULO8.etapa} · {MODULO8.etapaGloss}
          </p>
          <p className="mt-1 text-xs text-muted">
            Grado {MODULO8.etapaNum} de {MODULO8.etapasTotal} — el primer módulo del tercer nivel.
          </p>

          <div className="mt-7">
            <p className="font-cinzel text-[10px] uppercase tracking-[0.3em] text-gold/45">
              Módulo 8 · <span className="hebrew text-gold/70">{MODULO8.he}</span>
            </p>
            <h1 className="mt-1 font-cinzel text-2xl text-parchment sm:text-3xl">{MODULO8.titulo}</h1>
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted">
              Seis lecciones con la Mesilat Yesharim: el Mussar como práctica del alma. Zerizut, nekiut, anavá — las primeras tres escaleras del camino.
            </p>
          </div>
        </div>

        {/* aviso de bloqueo si M7 no está completo */}
        {!m7Complete && (
          <div className="mt-8 rounded-2xl border border-gold/20 bg-gold/[0.04] px-6 py-6 text-center">
            <p className="text-base text-parchment/80">
              Completa el Módulo 7 para desbloquear el Mussar.
            </p>
            <p className="mt-2 text-sm text-muted">
              Entrega las tareas de las 2 lecciones del Módulo 7 — cada una abre la siguiente puerta.
            </p>
            <div className="mt-5">
              <Link
        ...
        {/* progreso (solo visible si M7 está completo) */}
        {m7Complete && (
          <div className="mt-8">
            <div className="flex items-center justify-between font-cinzel text-[10px] uppercase tracking-[0.2em] text-gold/55">
              <span>{doneCount} de {MODULO8.total} lecciones</span>
              <span className="hebrew text-gold/50" dir="rtl">{MODULO8.auroraHe}</span>
            </div>
        ...
        {/* banner de finalización de J1 · Mussar */}
        {m8AllDone && (
          <div className="mt-8 rounded-2xl border border-gold/40 bg-gold/[0.07] px-6 py-7 text-center">
            ...
            <p className="mt-3 font-cinzel text-sm uppercase tracking-[0.3em] text-gold/80">
              Terminaste J1 · Mussar
            </p>
            <p className="mt-3 text-sm leading-relaxed text-parchment/80">
              Seis semanas. El trabajo del alma comenzó.
        ...
        <ol className="mt-8 flex flex-col gap-3">
          {LESSONS8.map((l) => {
            const isDone = m7Complete && completed.includes(l.slug);
            const isSeenLesson = m7Complete && !isDone && seen.includes(l.slug);
            const isCurrent = m7Complete && !m8AllDone && current === l.slug;
            const isLocked = !m7Complete;
