...
import { LESSONS10, MODULO10 } from "@/lib/academia/modulo10";
import { readProgress, isModuleComplete, M1_EVENT, type M1Progress } from "@/lib/academia/progress";

// M10 se desbloquea cuando los 5 slugs de M9 están completados.
const M9_SLUGS = [
  "ibn-ezra",
  "ramban",
  "tres-voces-bereshit",
  "remez-practica",
  "primera-lectura-pardes",
];

// Mapa del Módulo 10 — las 4 lecciones de J3 · Nombres divinos y Shemá (semanas 46–49).
export default function Modulo10Map() {
  ...
  const m9Complete = prog ? isModuleComplete(M9_SLUGS) : false;

  const completed = prog?.completed ?? [];
  const seen = prog?.seen ?? [];
  const doneCount = m9Complete
    ? LESSONS10.filter((l) => completed.includes(l.slug)).length
    : 0;
  const pct = (doneCount / MODULO10.total) * 100;

  const current = m9Complete
    ? LESSONS10.find((l) => !completed.includes(l.slug))?.slug ?? null
    : null;
  const m10AllDone = m9Complete && LESSONS10.every((l) => completed.includes(l.slug));
    ...
    <div className="relative min-h-screen overflow-hidden" dir="ltr">
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="flex items-center justify-between text-gold/70">
          ...
          <Link href="/academia/modulo-9" className="font-cinzel text-[11px] uppercase tracking-[0.2em] transition hover:text-gold">
            ← Módulo 9
          </Link>
        </div>
        ...
        <div className="mt-4 text-center">
          ...
          <p className="mt-2 font-cinzel text-sm uppercase tracking-[0.3em] text-gold/70">
            {MODULO10.etapa} · {MODULO10.etapaGloss}
          </p>
          <p className="mt-1 text-xs text-muted">
            Grado {MODULO10.etapaNum} de {MODULO10.etapasTotal} — el tercer módulo del tercer año.
          </p>

          <div className="mt-7">
            <p className="font-cinzel text-[10px] uppercase tracking-[0.3em] text-gold/45">
              Módulo 10 · <span className="hebrew text-gold/70">{MODULO10.he}</span>
            </p>
            <h1 className="mt-1 font-cinzel text-2xl text-parchment sm:text-3xl">{MODULO10.titulo}</h1>
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted">
              Cuatro lecciones: los siete Nombres divinos, YHVH y Elohim, la Shemá como metafísica, y el Alenu.
            </p>
          </div>
        </div>

        {/* aviso de bloqueo si M9 no está completo */}
        {!m9Complete && (
          <div className="mt-8 rounded-2xl border border-gold/20 bg-gold/[0.04] px-6 py-6 text-center">
            <p className="text-base text-parchment/80">
              Completa el Módulo 9 para desbloquear los Nombres divinos.
            </p>
            <p className="mt-2 text-sm text-muted">
              Entrega las tareas de las lecciones del Módulo 9 — cada una abre la siguiente puerta.
            </p>
            <div className="mt-5">
              <Link
        ...
        {/* progreso (solo visible si M9 está completo) */}
        {m9Complete && (
          <div className="mt-8">
            <div className="flex items-center justify-between font-cinzel text-[10px] uppercase tracking-[0.2em] text-gold/55">
              <span>{doneCount} de {MODULO10.total} lecciones</span>
              <span className="hebrew text-gold/50" dir="rtl">{MODULO10.auroraHe}</span>
            </div>
        ...
        {m10AllDone && (
          <div className="mt-8 rounded-2xl border border-gold/40 bg-gold/[0.07] px-6 py-7 text-center">
            ...
            <p className="mt-3 font-cinzel text-sm uppercase tracking-[0.3em] text-gold/80">
              Terminaste J3 · El Nombre y la unidad
            </p>
            <p className="mt-3 text-sm leading-relaxed text-parchment/80">
              Cuatro semanas. Siete Nombres. Una unidad radical.
        ...
        <ol className="mt-8 flex flex-col gap-3">
          {LESSONS10.map((l) => {
            const isDone = m9Complete && completed.includes(l.slug);
            const isSeenLesson = m9Complete && !isDone && seen.includes(l.slug);
            const isCurrent = m9Complete && !m10AllDone && current === l.slug;
            const isLocked = !m9Complete;
            ...
            return (
              <li key={l.slug}>
                {isLocked ? (
                  ...
                  <Link
                    href={`/academia/modulo-10/${l.slug}`}
