...
export default function LeccionView11({ lesson }: { lesson: Lesson }) {
  const [beat, setBeat] = useState(0);
  const [animKey, setAnimKey] = useState(0);
  const [javerDone, setJaverDone] = useState(false);
  const next = nextLesson11(lesson.slug);
  ...
  // Callback invocado por TareaInput cuando se entrega la tarea de L52.
        ...
        <header className="mb-8">
          ...
          <div className="mt-5 text-center">
            <p className="font-cinzel text-[10px] uppercase tracking-[0.3em] text-gold/45">
              Módulo 11 · Lección {lesson.n} de {MODULO11.total}
            </p>
            ...
            <span className="mt-3 inline-flex items-center gap-2 rounded-full border border-gold/25 bg-gold/[0.05] px-3 py-1">
              <span className="hebrew text-sm text-gold">{MODULO11.etapaHe}</span>
        ...
        <div key={animKey} className="animate-fade-up flex-1">
          ...
          {beat === 4 && (
            <>
              ...
              />
              {/* Botón de cierre de JAVER — aparece tras entregar la tarea de L52 */}
              {javerDone && (
                <div className="mt-10 flex flex-col items-center gap-4 text-center">
                  ...
                  <p className="font-cinzel text-sm uppercase tracking-widest text-gold/70">
                    Completaste JAVER — Año 1 de la Academia
                  </p>
                  <Link
...
function BeatSello({ lesson, next }: { lesson: Lesson; next: Lesson | null }) {
  const hilos = lesson.hilos.filter((h) => {
    const href = resolveThreadHref11(h);
    if (!href) return true;
    // Si el hilo apunta a la siguiente lección, no lo mostramos en el chip.
    if (next && href === `/academia/modulo-11/${next.slug}`) return false;
    return true;
  });
    ...
    <div>
      ...
      {lesson.closeModule && (
        <div className="mx-auto mt-7 max-w-xl rounded-2xl border border-gold/20 bg-ink/40 px-6 py-6 text-center">
          ...
          <p className="mt-1 font-cinzel text-[10px] uppercase tracking-[0.25em] text-gold/45">
            Cierre de J4 · Módulo 11 · Cierre de JAVER — Año 1
          </p>
      ...
      <div className="mt-10 text-center">
        {next ? (
          <>
            <Link
              href={`/academia/modulo-11/${next.slug}`}
          ...
          <>
            ...
            <p className="mt-3 text-xs text-muted">
              Entrega tu tarea — y el cierre de JAVER te espera.
            </p>
          </>
        )}
      </div>
...
function ThreadChip({ hilo }: { hilo: Hilo }) {
  const href = resolveThreadHref11(hilo);
