...
export default function LeccionView8({ lesson }: { lesson: Lesson }) {
  const [beat, setBeat] = useState(0);
  const [animKey, setAnimKey] = useState(0);
  const [j1Done, setJ1Done] = useState(false);
  const next = nextLesson8(lesson.slug);
  ...
  // Callback invocado por TareaInput cuando se entrega la tarea de L40.
        ...
        <header className="mb-8">
          ...
          <div className="mt-5 text-center">
            <p className="font-cinzel text-[10px] uppercase tracking-[0.3em] text-gold/45">
              Módulo 8 · Lección {lesson.n} de {MODULO8.total}
            </p>
            ...
            <span className="mt-3 inline-flex items-center gap-2 rounded-full border border-gold/25 bg-gold/[0.05] px-3 py-1">
              <span className="hebrew text-sm text-gold">{MODULO8.etapaHe}</span>
        ...
        <div key={animKey} className="animate-fade-up flex-1">
          ...
          {beat === 4 && (
            <>
              ...
              />
              {/* Botón de cierre de J1 — aparece tras entregar la tarea de L40 */}
              {j1Done && (
...
function BeatSello({ lesson, next }: { lesson: Lesson; next: Lesson | null }) {
  const hilos = lesson.hilos.filter((h) => {
    const href = resolveThreadHref8(h);
    if (!href) return true;
    // Si el hilo apunta a la siguiente lección, no lo mostramos en el chip.
    if (next && href === `/academia/modulo-8/${next.slug}`) return false;
    return true;
  });
    ...
    <div>
      ...
      {lesson.closeModule && (
        <div className="mx-auto mt-7 max-w-xl rounded-2xl border border-gold/20 bg-ink/40 px-6 py-6 text-center">
          ...
          <p className="mt-1 font-cinzel text-[10px] uppercase tracking-[0.25em] text-gold/45">
            Cierre de J1 · Módulo 8 · Cierre de JAVER J1
          </p>
      ...
      <div className="mt-10 text-center">
        {next ? (
          <>
            <Link
              href={`/academia/modulo-8/${next.slug}`}
          ...
          <>
            ...
            <p className="mt-3 text-xs text-muted">
              Entrega tu tarea — y el cierre de J1 te espera.
            </p>
          </>
        )}
      </div>
...
function ThreadChip({ hilo }: { hilo: Hilo }) {
  const href = resolveThreadHref8(hilo);
