...
import { LESSONS11, MODULO11 } from "@/lib/academia/modulo11";
import { readProgress, isModuleComplete, M1_EVENT, type M1Progress } from "@/lib/academia/progress";

// M11 se desbloquea cuando los 4 slugs de M10 están completados.
const M10_SLUGS = [
  "nombres-divinos",
  "yhvh-elohim",
  "la-shema",
...
// Mapa del Módulo 11 — las 3 lecciones del Sefer Yetzirah / J4 (semanas 50–52).
export default function Modulo11Map() {
  ...
  const m10Complete = prog ? isModuleComplete(M10_SLUGS) : false;

  const completed = prog?.completed ?? [];
  const seen = prog?.seen ?? [];
  const doneCount = m10Complete
    ? LESSONS11.filter((l) => completed.includes(l.slug)).length
    : 0;
  const pct = (doneCount / MODULO11.total) * 100;

  const current = m10Complete
    ? LESSONS11.find((l) => !completed.includes(l.slug))?.slug ?? null
    : null;
  const m11AllDone = m10Complete && LESSONS11.every((l) => completed.includes(l.slug));
    ...
    <div className="relative min-h-screen overflow-hidden" dir="ltr">
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="flex items-center justify-between text-gold/70">
          ...
          <Link href="/academia/modulo-10" className="font-cinzel text-[11px] uppercase tracking-[0.2em] transition hover:text-gold">
            ← Módulo 10
          </Link>
        </div>
        ...
        <div className="mt-6 text-center">
          <p className="font-cinzel text-[10px] uppercase tracking-[0.3em] text-gold/50">
            ...
            <span className="text-gold/70">Javer</span> — El compañero de estudio
          </p>
        </div>
        ...
        <div className="mt-4 text-center">
          ...
          <p className="mt-2 font-cinzel text-sm uppercase tracking-[0.3em] text-gold/70">
            {MODULO11.etapa} · {MODULO11.etapaGloss}
          </p>
          <p className="mt-1 text-xs text-muted">
            Grado {MODULO11.etapaNum} de {MODULO11.etapasTotal} — el cuarto y último módulo del tercer año.
          </p>

          <div className="mt-7">
            <p className="font-cinzel text-[10px] uppercase tracking-[0.3em] text-gold/45">
              Módulo 11 · <span className="hebrew text-gold/70">{MODULO11.he}</span>
            </p>
            <h1 className="mt-1 font-cinzel text-2xl text-parchment sm:text-3xl">{MODULO11.titulo}</h1>
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted">
              Tres lecciones: el Sefer Yetzirah, las diez sefirot, y el árbol de la vida. El umbral de la Cabalá.
            </p>
          </div>
        </div>

        {/* aviso de bloqueo si M10 no está completo */}
        {!m10Complete && (
          <div className="mt-8 rounded-2xl border border-gold/20 bg-gold/[0.04] px-6 py-6 text-center">
            <p className="text-base text-parchment/80">
              Completa el Módulo 10 para desbloquear el Sefer Yetzirah.
            </p>
            <p className="mt-2 text-sm text-muted">
              Entrega las tareas de las 4 lecciones del Módulo 10 — cada una abre la siguiente puerta.
            </p>
            <div className="mt-5">
              <Link
        ...
        {/* progreso (solo visible si M10 está completo) */}
        {m10Complete && (
          <div className="mt-8">
            <div className="flex items-center justify-between font-cinzel text-[10px] uppercase tracking-[0.2em] text-gold/55">
              <span>{doneCount} de {MODULO11.total} lecciones</span>
              <span className="hebrew text-gold/50" dir="rtl">{MODULO11.auroraHe}</span>
            </div>
        ...
        {/* banner de finalización de JAVER */}
        {m11AllDone && (
          <div className="mt-8 rounded-2xl border border-gold/40 bg-gold/[0.07] px-6 py-7 text-center">
            ...
            <p className="mt-3 font-cinzel text-sm uppercase tracking-[0.3em] text-gold/80">
              Completaste JAVER — Año 1
            </p>
            <p className="mt-3 text-sm leading-relaxed text-parchment/80">
              Cincuenta y dos semanas. Cuatro módulos. El umbral de la Cabalá, cruzado.
        ...
        {/* la escalera de las 3 lecciones */}
        <ol className="mt-8 flex flex-col gap-3">
          {LESSONS11.map((l) => {
            const isDone = m10Complete && completed.includes(l.slug);
            const isSeenLesson = m10Complete && !isDone && seen.includes(l.slug);
            const isCurrent = m10Complete && !m11AllDone && current === l.slug;
            const isLocked = !m10Complete;
            ...
            return (
              <li key={l.slug}>
                {isLocked ? (
                  ...
                  <Link
                    href={`/academia/modulo-11/${l.slug}`}
