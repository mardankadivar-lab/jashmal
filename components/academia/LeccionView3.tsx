...
export default function LeccionView3({ lesson }: { lesson: Lesson }) {
  const [beat, setBeat] = useState(0);
  const [animKey, setAnimKey] = useState(0);
  const [talmidDone, setTalmidDone] = useState(false);
  const next = nextLesson3(lesson.slug);
  ...
  // Callback invocado por TareaInput cuando se entrega la tarea de L16.
        ...
        <header className="mb-8">
          ...
          <div className="mt-5 text-center">
            <p className="font-cinzel text-[10px] uppercase tracking-[0.3em] text-gold/45">
              Módulo 3 · Lección {lesson.n} de {MODULO3.total}
            </p>
            ...
            <span className="mt-3 inline-flex items-center gap-2 rounded-full border border-gold/25 bg-gold/[0.05] px-3 py-1">
              <span className="hebrew text-sm text-gold">{MODULO3.etapaHe}</span>
        ...
        <div key={animKey} className="animate-fade-up flex-1">
          ...
          {beat === 4 && (
            <>
              ...
              />
              {/* Botón de cierre de TALMID — aparece tras entregar la tarea de L16 */}
              {talmidDone && (
...
function BeatSello({ lesson, next }: { lesson: Lesson; next: Lesson | null }) {
  const hilos = lesson.hilos.filter((h) => {
    const href = resolveThreadHref3(h);
    if (!href) return true;
    // Si el hilo apunta a la siguiente lección, no lo mostramos en el chip.
    if (next && href === `/academia/modulo-3/${next.slug}`) return false;
    return true;
  });
    ...
    <div>
      ...
      <div className="mt-10 text-center">
        {next ? (
          <>
            <Link
              href={`/academia/modulo-3/${next.slug}`}
          ...
          <>
            ...
            <p className="mt-3 text-xs text-muted">
              Entrega tu tarea — y el cierre de TALMID te espera.
            </p>
          </>
        )}
      </div>
...
function ThreadChip({ hilo }: { hilo: Hilo }) {
  const href = resolveThreadHref3(hilo);
