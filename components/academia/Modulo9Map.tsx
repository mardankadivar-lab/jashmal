...
import { LESSONS9, MODULO9 } from "@/lib/academia/modulo9";
import { readProgress, isModuleComplete, M1_EVENT, type M1Progress } from "@/lib/academia/progress";

// M9 se desbloquea cuando los 6 slugs de M8 están completados.
const M8_SLUGS = [
  "que-es-el-mussar",
  "cheshbon-ha-nefesh",
  "zerizut",
...
// Mapa del Módulo 9 — las 5 lecciones de J2 (semanas 41–45).
export default function Modulo9Map() {
  ...
  const m8Complete = prog ? isModuleComplete(M8_SLUGS) : false;

  const completed = prog?.completed ?? [];
  const seen = prog?.seen ?? [];
  const doneCount = m8Complete
    ? LESSONS9.filter((l) => completed.includes(l.slug)).length
    : 0;
  const pct = (doneCount / MODULO9.total) * 100;

  const current = m8Complete
    ? LESSONS9.find((l) => !completed.includes(l.slug))?.slug ?? null
    : null;
  const m9AllDone = m8Complete && LESSONS9.every((l) => completed.includes(l.slug));
    ...
    <div className="relative min-h-screen overflow-hidden" dir="ltr">
      ...
      <div className="relative z-10 mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="flex items-center justify-between text-gold/70">
          ...
          <Link href="/academia/modulo-8" className="font-cinzel text-[11px] uppercase tracking-[0.2em] transition hover:text-gold">
            ← Módulo 8
          </Link>
        </div>
        ...
        <div className="mt-4 text-center">
          ...
          <p className="mt-2 font-cinzel text-sm uppercase tracking-[0.3em] text-gold/70">
            {MODULO9.etapa} · {MODULO9.etapaGloss}
          </p>
          <p className="mt-1 text-xs text-muted">
            Grado {MODULO9.etapaNum} de {MODULO9.etapasTotal} — el segundo módulo de JAVER.
          </p>

          <div className="mt-7">
            <p className="font-cinzel text-[10px] uppercase tracking-[0.3em] text-gold/45">
              Módulo 9 · <span className="hebrew text-gold/70">{MODULO9.he}</span>
            </p>
            <h1 className="mt-1 font-cinzel text-2xl text-parchment sm:text-3xl">{MODULO9.titulo}</h1>
            <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-muted">
              Cinco lecciones: Ibn Ezra, Ramban y la primera lectura completa en PaRDeS.
            </p>
          </div>
        </div>

        {/* aviso de bloqueo si M8 no está completo */}
        {!m8Complete && (
          <div className="mt-8 rounded-2xl border border-gold/20 bg-gold/[0.04] px-6 py-6 text-center">
            <p className="text-base text-parchment/80">
              Completa el Módulo 8 para desbloquear Leer en profundidad.
            </p>
            <p className="mt-2 text-sm text-muted">
              Entrega las tareas de las 6 lecciones del Módulo 8 — cada una abre la siguiente puerta.
            </p>
            <div className="mt-5">
              <Link
        ...
        {/* progreso (solo visible si M8 está completo) */}
        {m8Complete && (
          <div className="mt-8">
            <div className="flex items-center justify-between font-cinzel text-[10px] uppercase tracking-[0.2em] text-gold/55">
              <span>{doneCount} de {MODULO9.total} lecciones</span>
              <span className="hebrew text-gold/50" dir="rtl">{MODULO9.auroraHe}</span>
            </div>
        ...
        {m9AllDone && (
          <div className="mt-8 rounded-2xl border border-gold/40 bg-gold/[0.07] px-6 py-7 text-center">
            ...
            <p className="mt-3 font-cinzel text-sm uppercase tracking-[0.3em] text-gold/80">
              Completaste J2 — Leer en profundidad
            </p>
            <p className="mt-3 text-sm leading-relaxed text-parchment/80">
              Cinco semanas. Ibn Ezra, Ramban y tu primera lectura en PaRDeS completo.
        ...
        <ol className="mt-8 flex flex-col gap-3">
          {LESSONS9.map((l) => {
            const isDone = m8Complete && completed.includes(l.slug);
            const isSeenLesson = m8Complete && !isDone && seen.includes(l.slug);
            const isCurrent = m8Complete && !m9AllDone && current === l.slug;
            const isLocked = !m8Complete;
